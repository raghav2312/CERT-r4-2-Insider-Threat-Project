...
def add_action_thisweek(act, columns, lines, act_handles, week_index, stop, firstdate, dname = 'r5.2'):
    thisweek_act = []
    # print(f"Performing add_action_thisweek on {act}...", end="\t")
    while True:
        # print(lines[act])
        if not lines[act]: 
            stop[act] = 1
        ...
        if dname in ['r6.1','r6.2'] and act in ['email', 'file','http'] and '"' in lines[act]:
            ...
            tmp = lines[act].split(',')
        # print(f"tmp: {tmp}")
        # print(f"time_convert(tmp[1], 't2wn', real_sd= firstdate): {time_convert(tmp[1], 't2wn', real_sd= firstdate)}")
...
def combine_by_timerange_pandas(week_start, week_end, dname = 'r4.2', dpath='/home/uottawa.o.univ/akund008/project/data/InsiderThreatTestDataset'):
    allacts =  ['device','email','file', 'logon', 'http'] # 'http'
    firstline = str(subprocess.check_output(['head', '-2', os.path.join(dpath,dname,'http.csv')])).split('\\n')[1]
    ...
    data_firstdate = datetime.strptime(firstdate, '%Y-%m-%d') + timedelta(weeks=week_start)
    data_firstdate = time_convert(data_firstdate, 'dt2date')
    # CHANGE 2.0
    # week_index = 0
    ...
    lines = {}
    stop = {}
    allacts_tqdm_bar = tqdm(allacts, leave=True)
    for act in allacts_tqdm_bar:
        allacts_tqdm_bar.set_description(f"Processing {act}")
        act_handles[act] = open(os.path.join(dpath,dname,act+'.csv'),'r')
        ### remove data before week_start...
        ...
        while True:
            if dname in ['r6.1','r6.2'] and act in ['email', 'file','http'] and '"' in lines[act]:
                ...
                tmp = lines[act].split(',')
            week_diff = time_convert(tmp[1], 't2wn', real_sd=data_firstdate)
            if week_diff < 0:
                lines[act] = act_handles[act].readline()
            else:
                break
        stop[act] = 0 # store stop value indicating if all of the file has been read
    allacts_tqdm_bar.reset()
    while (sum(stop.values()) < len(allacts)) and (week_index <= week_end):
        thisweekdf = pd.DataFrame()
        # print(f"Line 108 --> Week: {week_index}")
        for act in allacts_tqdm_bar:
            allacts_tqdm_bar.set_description(f"Processing {act}")
            if 'email' == act:
...
def getuserlist(week_start, week_end, dname = 'r4.2', psycho = True):
    ldap_path = '/home/uottawa.o.univ/akund008/project/data/InsiderThreatTestDataset/r4.2/LDAP/'
    allfiles =  [ldap_path+f1 for f1 in os.listdir(ldap_path) if os.path.isfile(ldap_path+f1)]
    alluser = {}
    alreadyFired = []
    ...
    if psycho and os.path.isfile("/home/uottawa.o.univ/akund008/project/data/InsiderThreatTestDataset/r4.2/psychometric.csv"):

        p_score = pd.read_csv("/home/uottawa.o.univ/akund008/project/data/InsiderThreatTestDataset/r4.2/psychometric.csv",delimiter = ',').values
        for id in range(len(p_score)):
            if p_score[id,1] in alluser.keys():
        ...
        if dname in ['r4.1','r4.2']:
            ...
            # print(f"df.columns: {[col for col in df.columns]}")
    ...
    w1 = pd.read_pickle(f"DataByWeek/{week_start}.pickle")
    w2 = pd.read_pickle(f"DataByWeek/{week_start+1}.pickle")
    user_pc_dict = pd.DataFrame(index=df.index)
    user_pc_dict['pcs'] = None  
    # print(f"Line 209 --> df.index: {df.index}")
    # print(f"Line 210 --> w1: {w1}")
    # print(f"Line 211 --> w2: {w2}")
    for u in df.index:
        pc = list(set(w1[w1['user']==u]['pc']) & set(w2[w2['user']==u]['pc']))
        # print(f"Line 212 --> pc: {pc}")
        user_pc_dict.at[u,'pcs'] = pc
    # print(f"Line 213 --> user_pc_dict: {user_pc_dict}")
...
def get_mal_userdata(week_start, week_end, data = 'r4.2', usersdf = None):
    ...
    if type(usersdf) != pd.core.frame.DataFrame:
        usersdf = getuserlist(week_start, week_end, data)
    usersdf['malscene']=0
    usersdf['mstart'] = None
    usersdf['mend'] = None
    ...
    for i in listmaluser.index:
        ...
        # print(f"Line 262 --> usersdf: {usersdf}")
...
def process_week_num(week, users, userlist = 'all', data = 'r4.2'):
    try:
        ...
        print(f"Exception in process_week_num: {e}")
...
if __name__ == "__main__":
    ...
    dname = '/home/uottawa.o.univ/akund008/project/data/InsiderThreatTestDataset/r4.2'
    data_ver = dname.split('/')[-1]
    if data_ver not in ['r4.1','r4.2','r6.2','r6.1','r5.1','r5.2']:
        raise Exception('Please put this script in and run it from a CERT data folder (e.g. r4.2)')
    #make temporary folders
    [os.mkdir(x) for x in ["tmp", "ExtractedData", "DataByWeek", "NumDataByWeek"] if not os.path.isdir(x)]
    ...
    # CHANGE 2.0    
    # numWeek = 73 if data_ver in ['r4.1','r4.2'] else 75 # only 73 weeks in r4.1 and r4.2 dataset
    numWeek = week_end - week_start + 1
    st = time.time()
    
    #### Step 1: Combine data from sources by week, stored in DataByWeek
    combine_by_timerange_pandas(week_start, week_end, data_ver)
    print(f"Step 1 - Separate data by week - done. Time (mins): {(time.time()-st)/60:.2f}")
    st = time.time()
    
    #### Step 2: Get user list
    users = get_mal_userdata(week_start, week_end, data_ver)
    ...
    with tqdm_joblib(desc='Processing Weeks...', total=numWeek):
       Parallel(n_jobs=numCores)(delayed(process_week_num)(i, users, data=data_ver) for i in range(week_start, week_end+1))
    ...
    for mode in ['week','day','session']:
        weekRange = list(range(week_start, week_end+1)) if mode in ['day', 'session'] else list(range(week_start, week_end+1))
        (ul, uf_dict, list_uf) = get_u_features_dicts(users, data= data_ver)
        
        Parallel(n_jobs=numCores)(delayed(to_csv)(i, mode, data_ver, ul, uf_dict, list_uf, subsession_mode) 
                                   for i in weekRange)

        all_csv = open('ExtractedData/'+f'w{week_start}to{week_end}'+mode+data_ver+'.csv','a')
        ...
        if mode == 'session' and len(subsession_mode) > 0:
            for k1 in subsession_mode:
                for k2 in subsession_mode[k1]:
                    all_csv = open('ExtractedData/'+f'w{week_start}to{week_end}'+mode+ k1 + str(k2) + data_ver+'.csv','a')
